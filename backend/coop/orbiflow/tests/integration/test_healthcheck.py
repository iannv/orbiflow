...
from orbiflow.runtime_info import backend_label, git_branch, runtime_info


class RuntimeInfoTests(SimpleTestCase):
    @patch.dict("os.environ", {}, clear=True)
    def test_local_defaults(self):
        self.assertIsNone(git_branch())
    ...
    def test_render_sandbox_env(self):
        ...
        self.assertEqual(
            runtime_info(status="ok"),
            {
                "status": "ok",
                "git_branch": "develop",
                "backend": "orbiflow-backend-sandbox",
            },
        )
...
class HealthcheckViewTests(TestCase):
    ...
    def test_healthcheck_reports_render_env(self):
        response = self.client.get(reverse("healthcheck"))
